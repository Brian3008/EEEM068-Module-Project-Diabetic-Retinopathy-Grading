...
from src.dataset import DRFolderDataset, get_transforms, get_sampler
from src.model import build_beit_model, get_class_weights, FocalLoss
...
BATCH_SIZE   = 32
NUM_EPOCHS   = 20
LR           = 3e-5            # back to 3e-5, more stable than 5e-5
WEIGHT_DECAY = 0.01
VAL_SPLIT    = 0.15
NUM_WORKERS  = 4
# ─────────────────────────────────────────────────────────────────────────────
...
def main():
    ...
    # Focal loss with soft class weights — best combo for imbalanced medical data
    ...
    # Layer-wise LR: backbone 10x slower than classifier head
    head_params     = list(model.classifier.parameters())
    head_ids        = set(id(p) for p in head_params)
    backbone_params = [p for p in model.parameters() if id(p) not in head_ids]

    optimizer = AdamW([
        {"params": backbone_params, "lr": LR * 0.1},   # 3e-6
    ...
    print("Done. Outputs saved to outputs_v4/")
