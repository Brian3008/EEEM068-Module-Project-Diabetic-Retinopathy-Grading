# eval.py — Full evaluation & visualisation suite
...
def load_best_model():
    ...
    try:
        ...
        # PyTorch 2.6 defaults to weights_only=True. Older checkpoints may
        # include numpy objects (e.g., thresholds) and require full unpickling.
        if "Weights only load failed" in str(e):
            ckpt = torch.load(cfg.BEST_MODEL, map_location=cfg.DEVICE, weights_only=False)
        else:
            raise
...
def full_evaluation(history=None):
    ...
    y_true, y_pred, y_prob = get_predictions(model, val_loader)

    thresholds = ckpt.get("qwk_thresholds", None)
    if thresholds is not None:
        thresholds = np.asarray(thresholds, dtype=np.float32)
        y_scores = probabilities_to_scores(y_prob)
        y_pred = scores_to_labels(y_scores, thresholds)
        print(f"🎯 Using tuned QWK thresholds: {np.round(thresholds, 3).tolist()}")
